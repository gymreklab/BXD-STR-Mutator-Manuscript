...
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
	...
	strain_info = strain_info %>%
	  ...
	  mutate(gen_inbreeding = if_else(bxd_id == 'BXD221', 3, gen_inbreeding)) %>%
	  mutate(gen_inbreeding = if_else(bxd_id == 'BXD222', 3, gen_inbreeding)) %>%
	  mutate(gen_inbreeding = if_else(bxd_id == 'BXD223', 4, gen_inbreeding)) %>%
	  mutate(gen_inbreeding = if_else(bxd_id == 'BXD224', 4, gen_inbreeding)) %>%
	  mutate(gen_inbreeding = if_else(bxd_id == 'BXD225', 3, gen_inbreeding)) %>%
	  mutate(gen_inbreeding = if_else(bxd_id == 'BXD226', 3, gen_inbreeding)) %>%
	  mutate(gen_inbreeding = if_else(bxd_id == 'BXD227', 2, gen_inbreeding)) 
...
```{r fig.width = 16, fig.height = 8, fig.align = 'center', cache = TRUE, eval = TRUE}
		...
		final_phenos_qtl = map(names(phenos), function(pt) {
		  ...
		  this_phenos = all_phenos %>%
			...
			# % de novo -> use gen inbreeding
			# all others -> no covariates
			if (pt == '% denovo') {
			 this_covar = strain_info %>% 
			   #filter(off_epoch != "epoch_7") %>% # problematic gen inbreeding for epoch 7. need to adjust TODO
...
perc_exp_byhap = all_phenos %>% 
  filter(min_pts_per_phe == 10, max_denovo_strains_per_loc == 10, motif_len == 'all') %>%
		    filter(metric == '% expanded') %>%
		    filter(strain %in% strains) %>%
...
p = perc_exp_byhap %>%
  ...
  geom_quasirandom(dodge.width = 0.1, width=0.2, groupOnX = TRUE) +
...
p
ggsave(path('pdfs/', 'Fig2_SignalByFounder.pdf'), p, w = 16, h = 8)
...
p = perc_exp_byhap %>%
  ...
  geom_quasirandom(dodge.width = 0.9, width=0.3, groupOnX = TRUE) +
  geom_boxplot(
					 fill = 'white', alpha=0, 
...
# Determine the QTL confidence interval and plot genes - TODO
