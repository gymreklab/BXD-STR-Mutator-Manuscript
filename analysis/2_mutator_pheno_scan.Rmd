...
# Plot founder gt at locus vs. expansion propensity (SuppFigure intergenic)
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
# founder genotypes at max LOD marker for %expanded 
fou_gt_at_peak = maxmarg(qtl_data$snp_probs[,'13'], minprob = 0.5) %>% 
		.[[1]] %>%
...
p = perc_exp_byhap %>%
  ...
  ylab("Expansion propensity")
p
ggsave(fs::path('../pdfs/', 'SuppFig_intergenic.pdf'), p, w = 6, h = 8)
```
# Load data 
...
# Plot founder gt at locus vs. expansion propensity (SuppFigure genic)
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
# founder genotypes at max LOD marker for %expanded 
fou_gt_at_peak = maxmarg(qtl_data$snp_probs[,'13'], minprob = 0.5) %>% 
		.[[1]] %>%
...
p = perc_exp_byhap %>%
  ...
  ylab("Expansion propensity")
p
ggsave(fs::path('../pdfs/', 'SuppFig_genic.pdf'), p, w = 6, h = 8)
```
