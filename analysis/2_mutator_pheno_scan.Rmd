...
# Visualize scan sensitivity results %expanded (Supp Figure 6)
...
perc_expand_final_qtl = all_phenos %>%
  ...
  summarise(qtl_res = map(pheno_vals, function(.x) {
	...
	p = by_chrom[[1]]
	p
	ggsave(fs::path('../pdfs/', 'SuppFig6_SensitivityScan.pdf'), p, w = 16, h = 16)
```

# Visualize %expanded by motif length (no filtering) (Supp Figure 7)
...
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
	...
	p
	ggsave(fs::path('../pdfs/', 'SuppFig7_PercExpByPeriod.pdf'), p, w = 6, h = 4)
```

# Visualize QTL signal by epoch (Supp Figure 8)
...
perc_expand_by_epoch_tetra_final = epoch_grps %>%
  ...
  write.csv(perc_expand_by_epoch_tetra_final, '../outs/perc_expand_by_epoch_tetra_final.csv')
	...
	p
	ggsave(fs::path('../pdfs/', 'SuppFig8_EpochScan-Tetra.pdf'), p, w = 16, h = 8)
```
...
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
	# join motif length to new variant STRs (note: these are homozygous)
  denovo_strs = readr::read_csv('../outs/denovo_strs_filtered_intergenic.csv')
	denovo_strs = denovo_strs %>%
	left_join(motif_info %>% select(chr, pos, end, motif_len), by = c('chr', 'pos', 'end'))
	strain_info = strain_info %>%
	  mutate(off_epoch = recode(off_epoch, epoch_1a = 'epoch_1', epoch_1b = 'epoch_1', epoch_1c = 'epoch_1')) 
...
# Plot founder gt at locus vs. expansion propensity (SuppFigure 4 intergenic)
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
# founder genotypes at max LOD marker for %expanded 
fou_gt_at_peak = maxmarg(qtl_data$snp_probs[,'13'], minprob = 0.5) %>% 
		.[[1]] %>%
...
p = perc_exp_byhap %>%
  ...
  ylab("Expansion propensity")
p
ggsave(fs::path('../pdfs/', 'SuppFig4_intergenic.pdf'), p, w = 6, h = 8)
```
# Load data 
...
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
	# join motif length to new variant STRs (note: these are homozygous)
  denovo_strs = readr::read_csv('../outs/denovo_strs_filtered_genic.csv')
	denovo_strs = denovo_strs %>%
	left_join(motif_info %>% select(chr, pos, end, motif_len), by = c('chr', 'pos', 'end'))
	strain_info = strain_info %>%
	  mutate(off_epoch = recode(off_epoch, epoch_1a = 'epoch_1', epoch_1b = 'epoch_1', epoch_1c = 'epoch_1')) 
...
# Plot founder gt at locus vs. expansion propensity (SuppFigure 4 genic)
```{r fig.width = 6, fig.height = 4, fig.align = 'center', cache = TRUE, eval = TRUE}
# founder genotypes at max LOD marker for %expanded 
fou_gt_at_peak = maxmarg(qtl_data$snp_probs[,'13'], minprob = 0.5) %>% 
		.[[1]] %>%
...
p = perc_exp_byhap %>%
  ...
  ylab("Expansion propensity")
p
ggsave(fs::path('../pdfs/', 'SuppFig4_genic.pdf'), p, w = 6, h = 8)
```
